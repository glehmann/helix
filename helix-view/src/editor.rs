...
impl Editor {
    ...
    fn refresh_vcs_watched_paths(&mut self) {
        let (workspace, _) = helix_loader::find_workspace();
        self.file_watcher
            .set_extra_watched_paths(self.diff_providers.get_watched_paths(&workspace));
    }
    ...
    pub fn open(&mut self, path: &Path, action: Action) -> Result<DocumentId, DocumentOpenError> {
        ...
        let id = if let Some(id) = id {
            ...
            self.refresh_vcs_watched_paths();
    ...
    pub fn save<P: Into<PathBuf>>(
        ...
        let save_path = path.clone().or_else(|| doc.path().map(PathBuf::from));
