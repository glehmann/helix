...
impl DiffProviderRegistry {
    ...
    pub fn needs_reload(&self, fs_event: &helix_core::file_watcher::Event) -> bool {
        self.providers
            .iter()
            .any(|(_, provider)| provider.needs_reload(fs_event))
    }
    ...
    pub fn get_watched_paths(&self, workspace: &Path) -> Vec<PathBuf> {
        self.providers
            .iter()
            .filter_map(|(_, provider)| provider.get_watched_path(workspace))
            .collect()
    }
...
impl DiffProvider {
    ...
    fn needs_reload(&self, _fs_event: &helix_core::file_watcher::Event) -> bool {
        match self {
            #[cfg(feature = "git")]
            Self::Git { .. } => {
                let path = _fs_event.path.as_std_path();
            ...
            #[cfg(feature = "jj")]
            Self::JJ(repo) => _fs_event.path.as_std_path() == jj::get_checkout_path(repo),
        }
    }
...
impl DiffProvider {
    #[allow(unreachable_patterns)]
    fn get_watched_path(&self, _workspace: &Path) -> Option<PathBuf> {
        match self {
            #[cfg(feature = "git")]
            Self::Git { .. } => git::get_head_path(_workspace),
            #[cfg(feature = "jj")]
            Self::JJ(repo) => Some(jj::get_checkout_path(repo)),
        }
    }
}
...
mod tests {
    ...
    #[cfg(feature = "git")]
    fn make_git_provider() -> DiffProvider {
        let dir = tempfile::tempdir().expect("temp dir for git test");
        ...
        let repo = git::open_repo(dir.path(), true).expect("open git repo");
        std::mem::forget(dir);
        DiffProvider::Git {
            repo: Box::new(repo),
            trust_full: true,
    ...
    fn test_needs_reload_regular_git() {
        ...
        let provider = make_git_provider();
    ...
    #[cfg(feature = "jj")]
    #[test]
    fn test_needs_reload_jj_checkout() {
        use helix_core::file_watcher::{CanonicalPathBuf, Event, EventType};
        use std::path::Path;

        let repo = Arc::from(Path::new("/home/user/repo"));
        let provider = DiffProvider::JJ(Arc::clone(&repo));
        let checkout = Event {
            path: CanonicalPathBuf::assert_canonicalized(&jj::get_checkout_path(&repo)),
            ty: EventType::Modified,
        };
        assert!(provider.needs_reload(&checkout));

        let other = Event {
            path: CanonicalPathBuf::assert_canonicalized(Path::new(
                "/home/user/repo/.jj/working_copy/tree_state",
            )),
            ty: EventType::Modified,
        };
        assert!(!provider.needs_reload(&other));
        assert_eq!(
            provider.get_watched_path(Path::new("/home/user/repo")),
            Some(jj::get_checkout_path(&repo))
        );
    }
}
